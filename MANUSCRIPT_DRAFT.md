...
A ToxT–DNA complex was predicted independently with the AlphaFold3 server and with Chai-1, both using the full-length ToxT sequence and the native El Tor *ctxAB* promoter duplex (36 bp, positions −76 to −41; transcribed and verified from Dittmer & Withey, 2012 — see Supplementary Section S.13 for the sequence and its verification). The top-ranked model from each tool (of five) was analysed; confidence was assessed by pTM/ptm and interface ipTM. The fatty-acid-bound crystal (3GBG) was superposed onto the modelled ToxT (Cα) to place the native lipid, protein–DNA contacts were defined at 5 Å, and interactions for both models were additionally profiled with PLIP 3.0.1 (default automatic ligand detection, which identifies hydrogen bonds, salt bridges and hydrophobic contacts from bond geometry). Given the modest-to-moderate interface confidence of both models (ipTM 0.48 and 0.67 respectively), both are reported in the Supplementary Information as exploratory structural checks rather than main-text results.
...
- **ToxT–DNA structural checks (Supplementary Information).** Reported only in the SI (Section S.13), not the main text: independent AlphaFold3 and Chai-1 models of ToxT bound to the native El Tor *ctxAB* promoter both place DNA contacts almost entirely in the C-terminal domain and none in the N-terminal fatty-acid pocket, but interface confidence is modest for AlphaFold3 (ipTM 0.48) and only moderately higher for Chai-1 (ipTM 0.67, still short of a conclusively confident threshold), and the duplex is modelled as an isolated 36-bp fragment without flanking genomic context, RNA polymerase or H-NS. These are exploratory checks on the known domain-separated architecture (Lowden et al., 2010), not an independently established binding geometry or mechanism for these lipids.
...
*Antivirulence strategy and antimicrobial resistance*
12. Allen RC, Popat R, Diggle SP, Brown SP. Targeting virulence: can we make evolution-proof drugs? *Nat Rev Microbiol* 2014;12:300–308.
13. Dickey SW, Cheung GYC, Otto M. Different drugs for bad bugs: antivirulence strategies in the age of antibiotic resistance. *Nat Rev Drug Discov* 2017;16:457–471.

*Computational methods and tools*
14. Trott O, Olson AJ. AutoDock Vina. *J Comput Chem* 2010;31:455–461.
15. Eberhardt J, Santos-Martins D, Tillack AF, Forli S. AutoDock Vina 1.2.0. *J Chem Inf Model* 2021;61:3891–3898.
16. Quiroga R, Villarreal MA. Vinardo scoring function. *PLoS ONE* 2016;11:e0155183.
17. Eastman P, et al. OpenMM 8: Molecular dynamics simulation with machine-learning potentials. *J Phys Chem B* 2024;128:109–116.
18. Boothroyd S, et al. Development and benchmarking of Open Force Field 2.0.0 (Sage). *J Chem Theory Comput* 2023;19:3251–3275.
19. Open Force Field Initiative. OpenFF NAGL: graph neural network partial charge assignment. https://github.com/openforcefield/openff-nagl *(software; no dedicated peer-reviewed publication identified as of this draft — verify before submission and cite the specific pinned version/release used, per Methods 2.10.)*
20. Abramson J, et al. Accurate structure prediction of biomolecular interactions with AlphaFold3. *Nature* 2024;630:493–500.
21. Chai Discovery. Chai-1: decoding the molecular interactions of life. *bioRxiv* 2024. doi:10.1101/2024.10.10.615955.
22. Schake P, Bolz SN, Linnemann K, Schroeder M. PLIP 2025: introducing protein–protein interactions to the protein–ligand interaction profiler. *Nucleic Acids Res* 2025;53(W1):W463–W465. doi:10.1093/nar/gkaf361.
23. McGibbon RT, et al. MDTraj: A modern open library for the analysis of molecular dynamics trajectories. *Biophys J* 2015;109:1528–1532.
24. Kim S, et al. PubChem 2023 update. *Nucleic Acids Res* 2023;51:D1373–D1380.
25. Landrum G, et al. RDKit: Open-source cheminformatics. https://www.rdkit.org.
...
*(2026-09-01: targeting submission to* npj Antimicrobials and Resistance*. Added a short AMR-relevance framing pass (one sentence in Abstract, one in Introduction, a new paragraph in Discussion) explaining why an antivirulence mechanism against a multidrug-resistant strain is relevant to the antimicrobial-resistance literature — antivirulence agents are argued to impose weaker selection pressure for resistance than bactericidal antibiotics, while noting explicitly that resistance to antivirulence compounds themselves is not impossible and untested here. Added two new references for this (Allen et al., 2014; Dickey et al., 2017), both verified via web search before citing. Also closed a pre-existing gap found while editing the reference list: Chai-1 and PLIP 3.0.1 were used and cited by name in Methods 2.12/Discussion/Limitations but had no bibliography entries — added both (Chai Discovery, 2024, bioRxiv; Schake et al., 2025, Nucleic Acids Res), also verified via web search. Reference list renumbered 1–25 (was 1–21) to accommodate all four additions; no other numbering or content changed.)*
