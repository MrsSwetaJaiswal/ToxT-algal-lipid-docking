...
- **Single receptor conformation and single trajectories.** Docking used one rigid crystal conformation. Each MD system was a single 50 ns trajectory; because the fatty-acid pocket is internal, ligand retention over 50 ns is a modest bar and not by itself strong evidence of favourable binding. Independent replicates (in progress) and longer sampling are needed to quantify stability and induced fit; the specificity argument therefore rests on binding preference (blind docking and energetics), not on MD retention (see decoy control below).
- **Decoy control and buried-pocket caveat.** The glucose decoy was tested from both a peripheral docked pose and seeded in the pocket core; because it is retained when core-seeded, MD stability does not by itself establish specificity for this internal pocket. The specificity conclusion therefore rests on binding preference (blind docking: 0/20 poses in the pocket; weak affinity) and energetics (MM-GBSA), which we consider the appropriate discriminators. An apo-versus-holo comparison would further test the proposed allosteric rigidification.
- **MM-GBSA.** Single-trajectory MM-GBSA (OBC, entropy omitted) yields relative, not absolute, energies; it converged only for the neutral/well-behaved systems, while charged (carboxylate) and some ester ligands showed large frame-to-frame variance and are reported with caution / in the supplement.
