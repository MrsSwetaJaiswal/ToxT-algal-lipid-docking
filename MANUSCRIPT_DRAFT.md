...
### 2.12 ToxT–DNA modelling (Supplementary Information)
A ToxT–DNA complex was predicted with the AlphaFold3 server using the full-length ToxT sequence and a 34-bp duplex containing two direct-repeat toxbox consensus elements (yrTTTTwwTwAww). The top-ranked model (of five) was analysed; confidence was assessed by pTM and interface ipTM. The fatty-acid-bound crystal (3GBG) was superposed onto the modelled ToxT (Cα) to place the native lipid, and protein–DNA contacts were defined at 5 Å. Given the low interface confidence of this model (Supplementary Figure S23), it is reported in the Supplementary Information as an exploratory structural check rather than a main-text result.
...
- **ToxT–DNA model (Supplementary Information).** Reported only in the SI (Section S.13, Figure S23), not the main text: the AlphaFold3 complex has a confident fold but a low-confidence DNA interface (ipTM 0.31) and used a consensus toxbox rather than the native promoter; the allosteric mechanism is a structural hypothesis grounded in the known domain architecture (Lowden et al., 2010), not a geometry established by our own modelling.
