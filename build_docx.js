...
kids.push(H("2.3 Ligand sets and 3D structure generation", HeadingLevel.HEADING_2));
...
kids.push(H("2.4 Binding-site (grid box) definition", HeadingLevel.HEADING_2));
...
kids.push(H("2.5 Docking", HeadingLevel.HEADING_2));
...
kids.push(H("2.6 Protocol validation, both-forms, consensus, and analysis", HeadingLevel.HEADING_2));
...
kids.push(H("2.7 Blind docking", HeadingLevel.HEADING_2));
...
kids.push(H("2.8 ToxT-DNA structural checks (Supplementary Information)", HeadingLevel.HEADING_2));
kids.push(P("A ToxT-DNA complex was predicted independently with the AlphaFold3 server and with Chai-1, both using the full-length ToxT sequence and the native El Tor ctxAB promoter duplex (36 bp, positions -76 to -41; transcribed and verified from Dittmer & Withey, 2012 -- see Supplementary Section S.13 for the sequence and its verification). The top-ranked model from each tool (of five) was analysed (confidence: pTM/ptm, interface ipTM); the fatty-acid-bound crystal (3GBG) was superposed onto the modelled ToxT (Ca) to place the native lipid, protein-DNA contacts were defined at 5 A, and interactions for both models were additionally profiled with PLIP 3.0.1. Given the modest-to-moderate interface confidence of both models (ipTM 0.48 and 0.67 respectively), both are reported in the Supplementary Information as exploratory structural checks rather than main-text results.", { justify: true }));
kids.push(H("2.9 Molecular dynamics", HeadingLevel.HEADING_2));
...
kids.push(P("Ligand binding reduced Ca RMSF modestly across the whole protein (apo 1.01 +/- 0.19 A, holo 0.88 +/- 0.11 A; mean over all 260 resolved residues), including at the fatty-acid pocket itself (apo 0.80 A, holo 0.64 A) and at five residues identified as DNA-proximal in exploratory ToxT-DNA modelling (Supplementary Section S.13) - Arg214, Lys235, Lys237, Tyr250, Lys256 (apo 0.95 A, holo 0.77 A). However, the magnitude of this reduction was similar in the C-terminal HTH domain (residues 188-273; -0.16 A) and the rest of the N-terminal domain (-0.12 A) (Figure 14), so the C-terminal domain is not disproportionately rigidified relative to the protein as a whole. This MD evidence indicates a modest, protein-wide stabilisation upon ligand binding, without disproportionate rigidification of any single domain.", { justify: true }));
...
kids.push(P("Exploratory AlphaFold3 and Chai-1 models of ToxT bound to the native El Tor ctxAB promoter are presented in the Supplementary Information (Section S.13) as structural checks on the known domain architecture, not as main-text results.", { justify: true }));
...
kids.push(P("This mechanism is particularly relevant given the antimicrobial-resistance context of the strain in which it was originally observed: the V. cholerae isolate used in our prior in vivo work (Jaiswal et al., 2025) is multidrug-resistant. Antivirulence strategies are increasingly discussed as a complementary approach to conventional antibiotics for exactly this scenario - because they disarm a pathogen's disease-causing machinery rather than killing it or halting its growth, they leave the bacterial growth pathways that classical antibiotics target (and that resistance mechanisms typically act on) untouched, and are argued to impose substantially weaker selection pressure for resistance (Allen et al., 2014; Dickey et al., 2017). This does not make antivirulence compounds resistance-proof - resistance to antivirulence agents themselves has been documented for other targets, and whether it would arise against ToxT specifically is untested here - but it does mean this mechanism offers a route to reducing virulence-driven disease burden in already multidrug-resistant strains without adding to the selective pressure that produced them in the first place.", { justify: true }));
...
kids.push(P("The independent MD replicate campaign originally planned here is now complete (n = 3 replicates per system across all 14 simulated systems; Section 2.9). Remaining planned extensions include MD of representative CCM binders and - contingent on quantitative GC-MS abundances - an abundance-weighted binding score to directly link the docking results to the organism-level bioassay.", { justify: true }));
...
[
 ...
 "Kumaran M, Palanisamy KM, Bhuyar P, Maniam GP, Rahim MHA, Govindan N. Agriculture of microalgae Chlorella vulgaris for polyunsaturated fatty acids (PUFAs) production employing palm oil mill effluents (POME) for future food, wastewater, and energy nexus. Energy Nexus 2023;9:100169. doi:10.1016/j.nexus.2022.100169.",
 "Allen RC, Popat R, Diggle SP, Brown SP. Targeting virulence: can we make evolution-proof drugs? Nat Rev Microbiol 2014;12:300-308.",
 ...
 "Schake P, Bolz SN, Linnemann K, Schroeder M. PLIP 2025: introducing protein-protein interactions to the protein-ligand interaction profiler. Nucleic Acids Res 2025;53(W1):W463-W465. doi:10.1093/nar/gkaf361.",
 ...
 "(2026-08-25: author-supplied strain designations and GC-MS methodology added (Section 2.1) -- CV accession (ATCC PTA-12198) and the full GC-MS protocol are now in the manuscript. CCM confirmed by the authors to have no culture-collection accession number (genuinely none available, not a missing placeholder). The apparent cis-10-heptadecenoic acid organism-assignment inconsistency flagged the same day was confirmed by the authors to be an error in the original Jaiswal et al. (2025) report; the compound is shared between CV and CCM, consistent with Tables 1/2 here.)",
 "(2026-08-26: full citation for ref 11 (Kumaran et al., 2023) supplied by the authors and filled into the reference list above. No open reference or author-supplied-content items remain in this manuscript.)"
